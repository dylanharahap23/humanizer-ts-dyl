...
exports.PARATACTIC_RAW_DRAFT_PROMPT = exports.PERSONAL_OBSERVATION_PROMPT = exports.GENUINE_HUMAN_REWRITE_PROMPT = exports.BLOG_STYLE_SECOND_PASS_PROMPT = exports.SELECTIVE_REWRITE_PROMPT = void 0;
exports.getSystemPromptByTone = getSystemPromptByTone;
exports.isFormalEssay = isFormalEssay;
exports.isGenericExplanation = isGenericExplanation;
exports.isFormalReligiousEssay = isFormalReligiousEssay;
exports.detectEnglishWritingProfile = detectEnglishWritingProfile;
exports.getEnglishHumanizerConfig = getEnglishHumanizerConfig;
exports.normalizeHumanizerTone = normalizeHumanizerTone;
exports.destroyThreeParagraphStructure = destroyThreeParagraphStructure;
exports.humanizeStructureEnglish = humanizeStructureEnglish;
exports.applyTargetedHumanImprint = applyTargetedHumanImprint;
exports.addHumanTouches = addHumanTouches;
exports.enhanceVocabulary = enhanceVocabulary;
exports.finalHumanize = finalHumanize;
exports.injectHumanSpecifics = injectHumanSpecifics;
exports.injectRealisticHumanFlaws = injectRealisticHumanFlaws;
exports.injectSafeSpecificsAndOrganicChaos = injectSafeSpecificsAndOrganicChaos;
exports.isShortGenericExplanation = isShortGenericExplanation;
exports.isComprehensiveNeutralExplanation = isComprehensiveNeutralExplanation;
exports.transformToPersonalOpinion = transformToPersonalOpinion;
const indonesian_humanizer_1 = require("./indonesian-humanizer");
// ============================================================
// 1. PROMPTS
// ============================================================
exports.SELECTIVE_REWRITE_PROMPT = `
...
// ============================================================
exports.BLOG_STYLE_SECOND_PASS_PROMPT = `
...
// ============================================================
exports.GENUINE_HUMAN_REWRITE_PROMPT = `
...
// ============================================================
exports.PERSONAL_OBSERVATION_PROMPT = `
...
// ============================================================
exports.PARATACTIC_RAW_DRAFT_PROMPT = `
...
// ============================================================
function getSystemPromptByTone(tone) {
    if (tone === "ielts") {
        return {
    ...
    return {
        ...
        additionalInstruction: "Make it feel human, conversational, and slightly bursty while preserving the original meaning.",
 ...
 */
function isFormalReligiousEssay(text) {
...
function detectEnglishWritingProfile(text, writingPurpose = "General") {
    ...
    const religiousTermCount = text.match(/\b(?:Allah|Qur['']an|Prophet(?:\s+Muhammad)?|hadiths?|haram|halal|duff|Hanbali|Maliki|Shafi['']?i|Hanafi|Islamic jurisprudence)\b/gi)?.length ?? 0;
    const clinicalTermCount = text.match(/\b(?:diagnosis|treatment|patient|clinical|medical condition|healthcare professional|physician|symptoms?|therapy)\b/gi)?.length ?? 0;
    const legalTermCount = text.match(/\b(?:statute|regulation|legal obligation|court|section|article of law|plaintiff|defendant|jurisdiction)\b/gi)?.length ?? 0;
    const policyWordCount = text.trim().split(/\s+/).filter(Boolean).length;
    const policyInstitutionCount = text.match(/\b(?:International Criminal Court|ICC|Rome Statute|arrest warrants?|jurisdiction|member states?|treaty obligations?|state cooperation|The Hague|Palestinian territories)\b/gi)?.length ?? 0;
    ...
    const firstPersonCount = text.match(/\b(?:I|me|my|mine|we|us|our|ours)\b/gi)?.length ?? 0;
    ...
    const hasCitation = /\[[0-9]{1,3}\]/.test(text) ||
    ...
    const hasPersonalPointOfView = /\b(?:I|me|my|mine|we|us|our|ours|you|your|yours)\b/i.test(text);
    const consumerDurableGoodsTopicCount = text.match(/\b(?:electric vehicles?|EVs?|laptop|smartphone|tablet|appliance|device|battery|charging|software updates?|resale value|dealer network|spare parts?|replacement components?|warranty|after-sales support|battery diagnostics?|manufacturer|ownership|hardware|repair costs?|lifespan|durability)\b/gi)?.length ?? 0;
    ...
    const reflectiveTopicCount = text.match(/\b(?:unemploy(?:ed|ment)|job loss|rejection|loneliness|lonely|isolation|grief|heartbreak|relationship|marriage|attraction|self-worth|confidence|financial insecurity|social comparison)\b/gi)?.length ?? 0;
    const reflectiveEmotionCount = text.match(/\b(?:painful|pain|difficult|hard|stress|anxiety|worry|worried|fear|frustration|embarrassed|judged|uncertainty|isolated|ashamed|shame|left behind)\b/gi)?.length ?? 0;
    ...
    const learningGuideTopicCount = text.match(/\b(?:focus(?:ed)?|attention|concentrat(?:e|ion)|learn(?:ing|ers?)|study(?:ing)?|working memory|cognitive overload|mental fatigue|distractions?)\b/gi)?.length ?? 0;
    const practicalLearningActionCount = text.match(/\b(?:break(?:ing)? information|review(?:ing)? material|connect(?:ing)? new knowledge|minimi[sz](?:e|ing) distractions?|study(?:ing)? in (?:short|focused) intervals?|summari[sz](?:e|ing)|ask(?:ing)? questions?|teach(?:ing)? others|scheduled breaks?|active learning|regular study habits?)\b/gi)?.length ?? 0;
    if (writingPurpose === "General" &&
    ...
    const stanceMarkerCount = text.match(/\b(?:should(?:\s+not|n't)?|must(?:\s+not|n't)?|need(?:s)?\s+to|have\s+to|ought\s+to)\b/gi)?.length ?? 0;
    const argumentSubjectCount = text.match(/\b(?:children|parents|society|people|students|workers|government)\b/gi)
        ?.length ?? 0;
    const careerChoiceCount = text.match(/\b(?:career|profession|doctor|medicine|engineering|business|education|arts|technology)\b/gi)?.length ?? 0;
    const hasArgumentContrast = /\b(?:but|while|rather than|instead of|not only|more likely)\b/i.test(text);
    const balancedFrameCount = text.match(/\b(?:on the one hand|on the other hand|although|while|however|therefore)\b/gi)?.length ?? 0;
    if (writingPurpose === "General" &&
        !hasPersonalPointOfView &&
        wordCount >= 100 &&
        stanceMarkerCount >= 1 &&
    ...
    if (/\b(your friend|you feel|don't compare|you shouldn't|you can|just because|feeling inferior|feel inferior|why can't I|why am I)\b/i.test(text) &&
        wordCount < 400) {
...
function getEnglishHumanizerConfig(sourceText, writingPurpose = "General") {
    ...
    if (profile === "consumer-explainer") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1,
            additionalInstruction: "Recompose the long-term buying analysis around the source's decision rule. Merge factor-by-factor sections, preserve every uncertainty and criterion, use second person only for the buying decision, and add no brand, model, anecdote, or outside fact.",
    ...
    if (profile === "policy-explainer") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1,
            additionalInstruction: "Recompose the legal-policy explanation around enforcement authority and practical dependency. Split dense clauses, remove factor-by-factor signposts, preserve every qualification, and add no outside case, opinion, or crime category.",
    ...
    if (profile === "sensitive") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1,
            additionalInstruction: "Use a source-anchored claim ledger, then recompose in plain English. Preserve every attribution, qualification, named authority, explicit answer, citation, and modal strength; never add a familiar detail that the source did not supply.",
    ...
    if (profile === "academic") {
        return {
            ...
            additionalInstruction: "Perform restrained academic editing. Prefer direct wording, keep natural repetition, and do not manufacture casualness, elevated synonyms, or personal experience.",
    ...
    if (profile === "reflective") {
        return {
            ...
            additionalInstruction: "Use a reader-oriented reflective voice from the source claims only. Conditional second person is allowed, but preserve every hedge, fact, example, and causal relationship; add no illustrative detail.",
    ...
    if (profile === "practical-explainer") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1.01,
            additionalInstruction: "Recompose the explanation as a source-faithful practical guide. Reader address is allowed, but add no personal story, authority, example, or recommendation absent from the source.",
    ...
    if (profile === "discursive") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1.01,
            additionalInstruction: "Use direct everyday English and regroup the source claims by idea. Preserve uncertainty and practical details exactly; do not add a narrator, rhetorical questions, anecdotes, or outside knowledge.",
    ...
    if (profile === "expository") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1.01,
            additionalInstruction: "Recompose from the source's claim units instead of rewriting sentence by sentence. Preserve the neutral point of view and factual scope, use ordinary vocabulary, and do not mirror the source's paragraph skeleton.",
    ...
    if (profile === "personal") {
        return {
            ...
            presencePenalty: 0,
            repetitionPenalty: 1.01,
            additionalInstruction: "Keep the narrator's real details and uneven pacing. Edit lightly and never invent autobiographical material.",
    ...
    if (profile === "argument-voice") {
        return {
            ...
            presencePenalty: 0.03,
            repetitionPenalty: 1.01,
            additionalInstruction: "Keep the stated position clear and source-faithful. Vary emphasis through the reasoning itself; do not invent a narrator, anecdote, question, typo, or dramatic aside.",
    ...
    if (profile === "product-review") {
        return {
            ...
            additionalInstruction: "Rewrite as a friendly product review with Q&A structure, headings, and direct reader address. Keep all facts but use everyday language.",
    ...
    if (profile === "personal-advice") {
        return {
            ...
            presencePenalty: 0.5,
            repetitionPenalty: 1.05,
            additionalInstruction: "Write like a close friend giving warm, simple advice. Use short sentences, fragments, repeated words for emphasis, and casual expressions. Avoid textbook language.",
            postProcessTone: "english-personal",
        };
    }
    ...
    return {
        ...
        presencePenalty: 0.2,
        repetitionPenalty: 1.03,
        additionalInstruction: "Write like a real person, not an AI. Use 'I' or 'you' where natural. Vary sentence length. Sound knowledgeable but relaxed.",
        postProcessTone: "english-general",
    };
}
function normalizeHumanizerTone(value) {
...
// ============================================================
function ensureMultiParagraph(text) {
...
function destroyThreeParagraphStructure(text) {
...
function humanizeStructureEnglish(text) {
    let paragraphs = text.split(/\n\s*\n/).filter(p => p.trim());
    if (paragraphs.length < 3)
        return text;
 ...
 */
function injectIdiosyncrasy(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 4)
        return text;
 ...
 */
function disruptStructure(text) {
    let paragraphs = text.split(/\n\s*\n/).filter(p => p.trim());
    if (paragraphs.length < 3)
        return text;
 ...
 */
function boostVocabularyEntropy(text) {
    const lowFrequencyMap = {
        'good': ['solid', 'decent'],
        'bad': ['rough', 'tricky'],
        'important': ['key', 'big'],
 ...
 */
function isGrammaticallyBroken(text) {
...
function injectTokenSurprise(text) {
    const surpriseMap = [
 ...
 */
function varyNumberExpression(text) {
    let result = text;
 ...
 */
function shiftToConversationalStyle(text, sourceText) {
    const sentences = splitSentences(text);
    if (sentences.length < 4)
        return text;
 ...
 */
function injectNaturalTypo(text) {
    // Only inject ONE typo per text, at most
    if (Math.random() > 0.2)
        return text; // 20% chance
    const commonTypos = [
 ...
 */
function targetedSentenceRestructure(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 5)
        return text;
 ...
 */
function applyTargetedHumanImprint(text, sourceText) {
...
// ============================================================
function enforceAggressiveBurstiness(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 4)
        return text;
...
function boostTokenPerplexity(text) {
    const perplexityMap = [
...
function diversifySentenceOpenings(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 4)
        return text;
    for (let i = 0; i < sentences.length - 2; i++) {
...
function humanizeReferences(text) {
    let result = text;
    const expansions = [
...
function calibrateHedging(text) {
    let result = text;
    result = result.replace(/\bmay (also )?(lead|cause|result|create|contribute)/gi, (m) => m.replace(/^may /i, 'sometimes '));
    // REMOVED: Hardcoded certainStatements yang jadi fingerprint tool
    // Filler contextual sekarang di-generate via LLM di second pass
    return result;
}
function varyDiscourseMarkers(text) {
    const markerMap = [
...
function injectGrammaticalAsymmetry(text) {
...
function normalizeNegatives(text) {
    return text
 ...
 */
function deJargonForCasualRegister(text, tone) {
...
function injectDifficultyVariance(text) {
    const rareWords = [
...
function createBurstinessOutlier(text) {
    let paragraphs = text.split(/\n\s*\n/).filter(p => p.trim());
    if (paragraphs.length < 2)
        return text;
...
function normalizeEmDashUsage(text) {
...
function varyInitialAdverbials(text) {
    const map = [
...
function humanizeSentenceSubjects(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 3)
        return text;
    let changes = 0;
...
function varyClosurePattern(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 3)
        return text;
...
function gentleBurstiness(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 4)
        return text;
...
function injectSpecificAnchors(text) {
    let result = text;
    // Decade/year anchors
...
function injectColloquialismByProfile(text, profile) {
    if (profile !== 'english-general' && profile !== 'casual')
        return text;
    let result = text;
    // Drop "very" sometimes
    result = result.replace(/\bvery\s+(\w+)/gi, (match, word) => {
        if (['good', 'bad', 'big', 'small', 'old', 'new'].includes(word.toLowerCase()))
            return word;
...
function recomposeSentenceOrder(text) {
...
function injectOneLongSentence(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 5)
        return text;
...
function aiFriendlyWordReplacement(text) {
    const map = [
...
// MASTER FUNCTION
function applyAntiDetectionPass(text, sourceText, tone) {
    if (!text || text.length < 100)
        return text;
    let result = text;
...
function applyStructuralChaos(text) {
    if (text.length < 100)
        return text;
    let result = text;
...
function shuffleNonCriticalSentences(text) {
    const paragraphs = text.split(/\n\s*\n/).filter(p => p.trim());
    if (paragraphs.length < 2)
        return text;
    const result = [];
...
function injectMildPersonalTouch(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 3)
        return text;
...
function casualizeTransitions(text) {
    const map = {
        Furthermore: "Plus",
        Moreover: "Also",
        "In addition": "Besides",
...
function adjustParagraphBreaks(text) {
    let paragraphs = text.split(/\n\s*\n/).filter(p => p.trim());
    if (paragraphs.length < 2)
        return text;
    const result = [];
...
function addHumanPunctuationFlaws(text) {
    return text
...
// ============================================================
function addHumanTouches(text, tone = "casual") {
    if (!text || text.length < 40)
        return text;
    let result = removeSyntheticEnglishHumanizerPhrases(text.trim());
    const usesPlainEnglish = tone === "casual" ||
...
// ============================================================
function simplifyInflatedEnglish(text) {
    const replacements = [
...
function makeReflectiveEnglishMoreDirect(text) {
    let result = text
    ...
    result = result.replace(/When someone[^.!?]*[.!?]/gi, (sentence) => sentence
...
function makeConsumerEnglishMoreDirect(text) {
    return text
...
function makePolicyEnglishMoreDirect(text) {
    return text
...
function makeDiscursiveEnglishMoreDirect(text) {
    let result = text
...
// 10. finalHumanize
// ============================================================
function finalHumanize(text, tone = "casual", skipHeavyProcessing = false) {
    if (tone === "indonesian-general" ||
        tone === "indonesian-academic" ||
        tone === "indonesian-professional") {
        return (0, indonesian_humanizer_1.finalIndonesianHumanize)(text, tone);
    }
    if (!text || text.length < 40)
        return text.trim();
...
function reduceRoboticHedging(text) {
    let result = text;
    const hedges = [
        [/\bmay be\b/gi, "is"],
...
function strengthenOpinionVoice(text) {
    let result = text;
    const opinionMap = [
...
function addNaturalRedundancy(text, chance) {
...
function varySentenceStarters(text, chance) {
    const sentences = splitSentences(text);
    if (sentences.length < 3)
        return text;
    const starters = [
    ...
    const varied = sentences.map((sentence) => {
        ...
        if (!["this", "it", "the", "there"].includes(firstWord) ||
            trimmed.length < 45 ||
            Math.random() > chance) {
            return sentence;
        }
...
function addConjunctionStarts(text, chance) {
    const sentences = splitSentences(text);
    if (sentences.length < 4)
        return text;
    const adjusted = sentences.map((sentence, index) => {
        if (index === 0 || Math.random() > chance)
            return sentence;
        const trimmed = sentence.trim();
        const firstWord = trimmed.split(/\s+/)[0]?.toLowerCase().replace(/[^a-z]/g, "");
...
function addNaturalImperfections(text, chance) {
    if (Math.random() > chance)
        return text;
    const sentences = splitSentences(text);
    if (sentences.length < 3)
...
function removeSyntheticEnglishHumanizerPhrases(text) {
    return text
        .replace(/(^|[.!?]\s+)(?:Honestly,\s+it matters|Plain and simple|And that's the thing|That's what I mean|You can feel the difference)\.\s*/gi, "$1")
...
function cleanupEnglishSpacing(text) {
    return text
        .replace(/\r\n/g, "\n")
        .split("\n")
        .map((line) => line
 ...
 */
function injectHumanSpecifics(text, sourceText) {
 ...
 */
function injectRealisticHumanFlaws(text) {
    const sentences = splitSentences(text);
    if (sentences.length < 3)
        return text;
 ...
 */
function injectSafeSpecificsAndOrganicChaos(text) {
 ...
 */
function isShortGenericExplanation(text) {
    const wordCount = text.split(/\s+/).filter(Boolean).length;
    if (wordCount > 250)
        return false; // longer texts already have room for specifics
 ...
 */
function isComprehensiveNeutralExplanation(text) {
    const paragraphs = text.split(/\n\s*\n/).filter(p => p.trim());
    if (paragraphs.length < 3)
        return false;
 ...
 */
function transformToPersonalOpinion(text) {
